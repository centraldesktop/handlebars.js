...
Handlebars.buildSource = function(template) {
  ...
  out += "return (" + Handlebars.buildRenderFromTemplate(template) + ")(Handlebars, context, helpers, partials);";
  out += "}";

  return out;
};
Handlebars.buildRenderFromTemplate = function(template) {
  out = "function(Handlebars, context, helpers, partials) {";

  out += "var container = Handlebars.VM.container();"
  out += "container.render = function(Handlebars, context, helpers, partials) {" + template.params[template.params.length - 1] + "};";

  for (var i = 0, j = template.children.length; i < j; i++) {
    out += "container.children << " + Handlebars.buildRenderFromTemplate(template.children[i]) + ";";
  }

  out += "return container.render(Handlebars, context, helpers, partials);";
