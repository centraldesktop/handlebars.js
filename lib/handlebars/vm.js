...
(function(Compiler, JavaScriptCompiler) {
  ...
  JavaScriptCompiler.prototype = {
    ...
    createFunction: function() {
      var container = Handlebars.VM.container();
      ...
      var template = Handlebars.VM.template(container);
			template.params = params;
			template.children = container.children;
...
// This is just useful for Alan for debugging - DELETE ME BEFORE THE COMPILER GETS RELEASED!
Handlebars.dumpSource = function(template, depth) {
	if (depth == undefined) {
		depth = 0;
	}
	...
	for (var i = 0,j=template.children.length; i < j; i++) {
		content = content + Handlebars.dumpSource(template.children[i], depth + 1);
	}
	
	return content + depth + " PARAMS: " + template.params.join(" : ") + "\n\n\n";
};

Handlebars.buildSource = function(template) {
  out = "function(context, helpers, partials) {";
  out += "helpers = helpers || Handlebars.helpers; partials = partials || Handlebars.partials;";
  out += "return (" + Handlebars.buildRenderFromTemplate(template) + ")(context, helpers, partials);";
  out += "}";

  return out;
};
Handlebars.buildRenderFromTemplate = function(template) {
  out = "function(context, helpers, partials) {";

  out += "var container = Handlebars.VM.container();"
  out += "container.render = function(context, helpers, partials) {" + template.params[template.params.length - 1] + "};";

  for (var i = 0, j = template.children.length; i < j; i++) {
    out += "container.children.push(" + Handlebars.buildRenderFromTemplate(template.children[i]) + ");";
  }

  out += "return container.render(context, helpers, partials);";
